...
void find_dev_usb()
{
  DIR* dir = opendir("/dev");
  struct dirent* drnt;
  log_write("checking for /dev/tty* files..\n");
  while((drnt = readdir(dir))){
    if(drnt->d_type == DT_CHR) {
      char* dname = drnt->d_name;
      if(!strncmp(dname, "tty", 3)) log_write("found tty device: %s\n", dname);
    }
  }
}
...
void serial_init(uart_rx_handler_t cb, uint32_t baudrate)
{
  ...
  find_dev_usb();
  char* devtty = "/dev/ttyACM0"; // .. but only root can see that
