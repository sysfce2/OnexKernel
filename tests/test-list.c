...
void test_list()
{
  ...
  onex_assert_equal(item_to_text(list_get(li,1),buf,128), "y",  "1st item is y");
  onex_assert_equal(item_to_text(list_get(li,2),buf,128), "3",  "2nd item is 3");
  onex_assert(     !list_get(li,3),       "3rd item is null");

                    list_set(li,2,(item*)value_new("5"));

  onex_assert(      list_size(li)==2,     "size should still be 2");
  onex_assert_equal(item_to_text(list_get(li,2),buf,128), "5",  "2nd item is now 5");

  onex_assert(     !list_set(li,3,(item*)value_new("+")), "can't set item out of range");
  ...
  onex_assert(      list_set(li,3,(item*)value_new("+")), "now can set 3rd item");
